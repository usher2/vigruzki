        ...
        if os.path.exists(filename):
                ...
                if uniqid not in alist:
                        if zipfile.is_zipfile(filename):
                                myxml = mysig = ""
                                tempdir = os.path.join(config['UPLOAD_FOLDER'], str(uuid.uuid4()))
                                with zipfile.ZipFile(filename) as myzip:
                                        try:
                                                ...
                                                args = ["borg", "create", "--noatime", "--noctime", "--timestamp", _ts, "--compression", "lzma", "--umask=0022", borg_repo + "::" + uniqid, "."]
                                                print("B: %s" % ' '.join(args))
